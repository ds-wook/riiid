...
def train_preprocessing(path: str) -> pd.DataFrame:
    ...
    train_df = dt.fread(
        path + "train.csv", columns=set(data_types_dict.keys())
    ).to_pandas()
    ...
    train_lectures = train_df[train_df.content_type_id == 1].merge(
    ...
    for column in user_lecture_stats_part.columns:
        if column != "user_id":
            user_lecture_stats_part[column] = (
                user_lecture_stats_part[column] > 0
            ).astype("int8")
    ...
    user_lecture_agg = train_df.groupby("user_id")["content_type_id"].agg(
        ["sum", "count"]
    )
    ...
    train_df["user_interaction_timestamp_mean"] = train_df[
    ...
    attempt_no_agg = (
        train_df.groupby(["user_id", "content_id"])["attempt_no"]
        .agg(["sum"])
        .astype("int8")
    )
    train_df["attempt_no"] = (
    ...
    max_timestamp_u2 = (
        train_df[["user_id", "lagtime"]].groupby(["user_id"]).agg(["max"]).reset_index()
    )
    ...
    delta_prior_question_elapsed_time_mean = train_df[
        "delta_prior_question_elapsed_time"
    ].mean()
    train_df["delta_prior_question_elapsed_time"].fillna(
        delta_prior_question_elapsed_time_mean, inplace=True
    )
    train_df.delta_prior_question_elapsed_time = (
    ...
    cum = train_df.groupby("user_id")["prior_question_had_explanation"].agg(
        ["cumsum", "cumcount"]
    )
