...
def gettags(tags: str, num: int) -> str:
    tags_splits = tags.split(" ")
    result = " ".join([t for t in tags_splits if 32 * num <= int(t) < 32 * (num + 1)])
...
def questions_feature_engine(path: str) -> pd.DataFrame:
    ...
    for num in range(6):
        questions_df["tags" + str(num)] = questions_df["tags"].apply(
            lambda row: gettags(row, num)
        )

        le = LabelEncoder()
        le.fit(np.unique(questions_df["tags" + str(num)].values))

        questions_df["tags" + str(num)] = questions_df[["tags" + str(num)]].apply(
            le.transform
        )
    ...
    questions_df = pd.merge(
        questions_df, questions_cmnts, on="question_id", how="left", right_index=True
    )

    questions_df.rename(columns={"question_id": "content_id"}, inplace=True)

    content_explation_agg = (
        train_df[["content_id", "prior_question_had_explanation", target]]
    ...
    questions_df.content_correctness = questions_df.content_correctness.astype(
        "float16"
    )
    ...
    questions_df["content_uncorrect_count"] = (
        questions_df["content_id"]
        .map(content_agg["count"] - content_agg["sum"])
        .astype("int32")
    )
    ...
    content_elapsed_time_agg = train_df.groupby("content_id")[
        "prior_question_elapsed_time"
    ].agg(["mean"])
    content_had_explanation_agg = train_df.groupby("content_id")[
        "prior_question_had_explanation"
    ].agg(["mean"])

    questions_df["content_elapsed_time_mean"] = questions_df["content_id"].map(
        content_elapsed_time_agg["mean"]
    )
    questions_df.content_elapsed_time_mean = (
        questions_df.content_elapsed_time_mean.astype("float16")
    )
    questions_df["content_had_explanation_mean"] = questions_df["content_id"].map(
        content_had_explanation_agg["mean"]
    )
    ...
    questions_df.part_correctness_std = questions_df.part_correctness_std.astype(
        "float16"
    )

    part_agg = questions_df.groupby("part")["content_uncorrect_count"].agg(["sum"])
    questions_df["part_uncor_count"] = (
    ...
    questions_df["bundle_correctness_mean"] = questions_df["bundle_id"].map(
        bundle_agg["mean"]
    )
    questions_df.bundle_correctness_mean = questions_df.bundle_correctness_mean.astype(
        "float16"
    )
